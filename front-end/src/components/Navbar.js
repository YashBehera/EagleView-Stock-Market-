...
export default function Navbar({ token }) {
  ...
  const toggleDropdown = () => {
    ...
    if (!hasRegistered) {
      setHasRegistered(true); // Mark user as having viewed the profile or registered
    }
  };
        ...
        <button onClick={toggleDropdown}>
          ...
          {userDetails ? (
            ...
            {userDetails ? (
              ...
              <div>
              <CgProfile className="flex ml-8 justify-center items-center text-black ml-5" />
              <span className="font-light text-xs flex justify-center text-black mr-[28px] ">
                Login/SignUp
              </span>
              </div>
            )}
